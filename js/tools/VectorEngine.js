...
window.VectorEngine = (() => {
    ...
    function isNodeMode() {
        if (window._illuForceNodeMode) return true;
    ...
    function refreshSelectionUIImpl() {
        ...
        if (typeof window.illuUpdateVectorPropertiesUI === 'function') {
            window.illuUpdateVectorPropertiesUI();
        }
    ...
    function _applyDrag(el, base, dx, dy) {
        ...
        try {
            ...
            const ctm = p ? p.getCTM() : el.ownerSVGElement.getCTM();
            if (ctm) {
    ...
    function updateDrag(pos) {
        ...
        if (isNodeMode() && _nodeSnapshots.length) {
            _nodeSelection.forEach(sel => {
                ...
                let ldx = dx, ldy = dy;
                try {
                    const ctm = targetEl.parentElement ? targetEl.parentElement.getCTM() : targetEl.ownerSVGElement.getCTM();
                    if (ctm) {
                        const inv = ctm.inverse();
                        ldx = dx * inv.a + dy * inv.c;
                        ldy = dx * inv.b + dy * inv.d;
